...
public class SchoolDetailChangeController implements Initializable {
    ...
    @FXML
    private TextField headmMail;

    @FXML
    private TextField secPhone;

    @FXML
    private TextField secMail;

    @FXML
    private TextField teachName;

    @FXML
    private TextField teachMail;
    ...
    void saveRecord(ActionEvent event) {
        ...
        String SQL = "UPDATE school SET name = ?,city = ?,region = ?, num_of_students = ?, language = ?," +
                     "headmaster_name = ?, headmaster_phone = ?, headmaster_mail = ?, secretary_phone = ?," +
                     "secretary_mail = ?, our_teacher_name = ?, our_teacher_mail = ?" +
                     "WHERE school_id = " + schoolID.toString() + ";";

        try {
            Connection conn = new PostgresConn().connect();
            PreparedStatement pstmt = conn.prepareStatement(SQL);
            pstmt.setString(1,schName.getText());
            pstmt.setString(2,city.getText());
            pstmt.setString(3,region.getText());
            pstmt.setInt(4,Integer.parseInt(studentCount.getText()));
            pstmt.setString(5,language.getText());
            pstmt.setString(6,headmName.getText());
            pstmt.setString(7,headmPhone.getText());
            pstmt.setString(8,headmMail.getText());
            pstmt.setString(9,secPhone.getText());
            pstmt.setString(10,secMail.getText());
            pstmt.setString(11,teachName.getText());
            pstmt.setString(12,teachMail.getText());
            pstmt.executeUpdate();
        }
